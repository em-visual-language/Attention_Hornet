...
class Language(nn.Module):
    def __init__(self):
        ...
        self.multiHheadattention = MultiHeadedAttention(4, self.hidden_dim, dropout=1)
    ...
    def forward(self, x, y):
        ...
        for (i,yt) in enumerate(y_emb.permute(1,0,2)):
            ...
            ###print('output:', output.size()) #(32,1,512)

            # Gate features combining h of bottom LSTM and image feature
            h_emb = self.emb_h(h0.squeeze())
            ###print('h_emb:', h_emb.size()) #(32,512)
            x_emb = self.emb_x(x)
            ###print('x_emb:', x_emb.size()) #(32,2048)
            ...
            ###print('h_top_0:',h_top_0.size()) #(1, 32, 512)
            ###print('output_top:', output_top.size()) #(32,1,512)
        ...
        h_cat_temp = torch.cat(HTop, 1)

        # Attention
        ###query = h_cat_temp
        ###key = h_cat_temp
        ###value = h_cat_temp
        ###h_cat = self.multiHheadattention(query, key, value)
        # average pooling
        ###print('h_cat:', h_cat.size()) #(32, 30, 512)
        h_cat=h_cat_temp
        h_mean = torch.mean(h_cat, dim=1).cuda()
        pred = h_mean
        ###print(pred.size()) #(32, 512)
